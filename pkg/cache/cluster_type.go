...
// All return all the original resources in the map, this maybe cause pointer leaks
func (m *ResourceMap) All() map[kube.ResourceKey]*Resource {
	result := make(map[kube.ResourceKey]*Resource)
	m.Range(func(key kube.ResourceKey, value *Resource) bool {
		result[key] = value
		return true
	})
	return result
...
func (l *APIResourceList) Add(info kube.APIResourceInfo) {
	l.lock.Lock()
	defer l.lock.Unlock()
	l.list = append(l.list, info)
}

func (l *APIResourceList) Get() []kube.APIResourceInfo {
	l.lock.RLock()
	defer l.lock.RUnlock()
	return l.list
}

func (l *APIResourceList) Len() int {
	l.lock.RLock()
	defer l.lock.RUnlock()
	return len(l.list)
}
...
func (l *APIResourceList) Remove(info kube.APIResourceInfo) bool {
	l.lock.Lock()
	defer l.lock.Unlock()
...
func (l *APIResourceList) Clear() {
	l.lock.Lock()
	defer l.lock.Unlock()
	l.list = []kube.APIResourceInfo{}
}

// GetReferrerList return real inner list of APIResourceList
func (l *APIResourceList) GetReferrerList() []kube.APIResourceInfo {
	l.lock.Lock()
	defer l.lock.Unlock()
	return l.list
}

// All return all the resources copy in the map, so it's lock free
func (l *APIResourceList) All() []kube.APIResourceInfo {
	l.lock.Lock()
	defer l.lock.Unlock()
	// shallow copy
	return l.list[:]
...
// AddIfAbsent return true if added
func (l *APIResourceList) AddIfAbsent(info kube.APIResourceInfo) bool {
	l.lock.Lock()
	defer l.lock.Unlock()
